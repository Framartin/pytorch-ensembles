...
#SBATCH --qos=long
#SBATCH -J cSGLDeffnet    # Set the job name
...
#SBATCH -o "log/run_train_imagenet_csgld_efficientnet_resume2_%j.log"
...
LR=0.01  # resume after 1rst cycle with smaller LR
CYCLES=3
SAMPLES_PER_CYCLE=3
BATCH_SIZE=128 # increase batch-size for efficiency
WORKERS=8
PRINT_FREQ=400
...
python -u train_imagenet_csgld.py --data $DATAPATH --no-normalization --arch $ARCH \
  ...
  --resume "../models/ImageNet/efficientnet_b0/cSGLD_cycles3_samples3_bs64/ImageNet-cSGLD_efficientnet_b0_00044.pt.tar"
