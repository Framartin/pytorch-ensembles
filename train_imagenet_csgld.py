...
parser.add_argument('--lr', '--learning-rate', default=0.1, type=float,
                    metavar='LR', help='initial learning rate (useful on restarts, set the same than --max_lr if training from scratch)', dest='lr')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    M = args.cycles
    T = epochs * epoch_batches

    def adjust_learning_rate_(optimizer, epoch, batch_idx):
        rcounter = epoch*epoch_batches+batch_idx
        cos_inner = np.pi * (rcounter % (T // M))  # pi * (what iteration of cycle I am on)
        cos_inner /= T // M  # 0..pi depending on where I am in the cycle
...
def train(train_loader, model, criterion, optimizer, epoch, adjust_learning_rate_, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if hasattr(output, 'logits'):
            output = output.logits  # for googlenet
...
def validate(val_loader, model, criterion, args, ret_lp=False):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # compute output
            output = model(images)
            log_prob = nn.LogSoftmax()(output)
            preds.append(log_prob.cpu().data.numpy())
            loss = criterion(output, target)
